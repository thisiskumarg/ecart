...
?>
    ...
    <div class="col-sm-10">
        ...
        <div class="row">
            ...
            if($roleid == '1')
            {
                $sql = "SELECT pid FROM products WHERE is_verified = '1'";
                $res = mysqli_query($con, $sql);
            }
            elseif($roleid == '2')
            {
                $sql = "SELECT pid FROM products WHERE is_verified = '1' AND uid = '$uid'";
                $res = mysqli_query($con, $sql);
            }
            foreach($res as $row)
            {
            ...
            ?>
            <h5 class="text-muted my-3">Verified Products : <span class="text-danger"><?php echo $vp; ?></span></h5>
            <hr>
        </div>
        <form action="db.php" method="post">
            <div class="row col-sm-12 mb-3">
                <?php 
                $status = '1';
                echo '<input type="hidden" name="pstatus" value='.$status.'>';
                ?>
                <input type="submit" value="SUSPEND" name="svp" class="btn btn-warning mx-2">
                <input type="submit" value="REMOVE" name="rvp" class="btn btn-danger">
            </div>
            <table class="table table-hover">
                ...
                <tbody>
                    ...
                    <tr class="table-hover">
                        <td><input type="checkbox" name="selectvp[]" value="<?php echo $row['pid']; ?>"></td>
                        <td><img src="<?php echo $row['pimage']; ?>" alt="Verified-Product-Photo" class="img-fluid" style="height: 100px; width: 130px;"></td>
                        ...
                        <td><a href="db.php?editproductform=<?php echo $row['pid']; ?>" class="btn btn-primary">EDIT</a></td>
                    </tr>
                    <?php 
                    }
                    ?>
    ...
    <!-- Modal-2 Start, Suspend Message -->

    <div class="modal fade" id="myModal2">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header bg-primary">
                    <h4 class="modal-title text-white">Verified Products Page says :</h4>
                    <button type="button" class="close" data-dismiss="modal">&times;</button>
                </div>
                <div class="modal-body text-primary">
                    <p>Product is <span class="text-warning">suspended</span> successfully.</p>
                </div>
            </div>
        </div>
    </div>

    <!-- Modal-2 End, Suspend Message -->
    ...
    <div class="modal fade" id="myModal3">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body text-primary">
                    <p>Product is <span class="text-danger">removed</span> successfully.</p>
                </div>
            </div>
        </div>
    </div>
...
{
    checkboxes=document.getElementsByName('selectvp[]');
